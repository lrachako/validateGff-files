...
if ($f[2] eq "gene") {$f[2] = "0" . $f[2];}
if ($f[2] eq "mRNA") {$f[2] = "1" . $f[2];}
if ($f[2] eq "exon") {$f[2] = "2" . $f[2];}
if ($f[2] eq "CDS") {$f[2] = "3" . $f[2];}
...
sub byNSE
{
  $name[$a] cmp $name[$b] or $start[$a] <=> $start[$b] or $type[$a] cmp $type[$b] or $end[$a] <=> $end[$b]; 
}
